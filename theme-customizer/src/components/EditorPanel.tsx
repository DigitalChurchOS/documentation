...
export const EditorPanel: React.FC<EditorPanelProps> = ({
  ...
  // Menus tab state
  const [expandedMenuName, setExpandedMenuName] = useState<string | null>("Header Menu");
  const [editingItemIndex, setEditingItemIndex] = useState<number | null>(null);
  const [draggedItemIndex, setDraggedItemIndex] = useState<number | null>(null);

  const handleMenusUpdate = (menuName: string, updatedItems: any[]) => {
    const updatedMenus = navigationMenus.map(m => {
  ...
  const handleItemFieldChange = (menuName: string, itemIdx: number, field: string, value: string) => {
    const targetMenu = navigationMenus.find(m => m.name.toLowerCase() === menuName.toLowerCase());
    if (!targetMenu) return;
    const items = typeof targetMenu.items === 'string' ? JSON.parse(targetMenu.items) : [...targetMenu.items];
    items[itemIdx] = {
      ...items[itemIdx],
      [field]: value
    };
    handleMenusUpdate(menuName, items);
  };

  const handleAddLink = (menuName: string) => {
    const targetMenu = navigationMenus.find(m => m.name.toLowerCase() === menuName.toLowerCase());
    if (!targetMenu) return;
    const items = typeof targetMenu.items === 'string' ? JSON.parse(targetMenu.items) : [...targetMenu.items];
    items.push({ label: "New Link", url: "/" });
    handleMenusUpdate(menuName, items);
    setEditingItemIndex(items.length - 1);
  };

  const handleDeleteLink = (menuName: string, itemIdx: number) => {
    const targetMenu = navigationMenus.find(m => m.name.toLowerCase() === menuName.toLowerCase());
    if (!targetMenu) return;
    const items = typeof targetMenu.items === 'string' ? JSON.parse(targetMenu.items) : [...targetMenu.items];
    const filtered = items.filter((_: any, idx: number) => idx !== itemIdx);
    handleMenusUpdate(menuName, filtered);
  ...
  const handleDropItem = (e: React.DragEvent, targetIndex: number, menuName: string) => {
    e.preventDefault();
    ...
    handleMenusUpdate(menuName, reordered);
  };
  ...
  useEffect(() => {
    if (selectedElement) {
      if (selectedElement.type === "header") {
        ...
        setSubView("footer");
      } else if (selectedElement.type === "rail") {
        setSubView("rail");
      } else {
        setSubView("element");
      }
  ...
  if (subView === "sections") {
    return (
      <div className="sidebar-view">
        <div className="mode-tabs sharp-tabs" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
          <button
          ...
          <button
            className={`mode-tab ${editorSubTab === "menus" ? "active" : ""}`}
            onClick={() => setEditorSubTab("menus")}
          >
            Menus
          </button>
        </div>
        ...
        <div className="panel">
          {editorSubTab === "sections" && (
            <>
              ...
              {sections.length > 0 ? (
                <>
                  {/* Header Sections */}
                  {sections.filter(s => s.type === "header").map((_, idx) => (
                    <button
                      key={`header-${idx}`}
                    ...
                    >
                      ...
                      <span>Header Layout & Navigation styles</span>
                    </button>
                  ))}

                  {/* Rail Navigation Customizer Card */}
                  <button
                    className="editor-card"
                    onClick={() => {
                      setSubView("rail");
                  ...
                  >
                    <strong>Rail Navigation</strong>
                    <span>Sidebar Rail layout, side alignment & icons toggle</span>
                  </button>
                  ...
                  {sections.filter(s => s.type !== "header" && s.type !== "footer").map((section, idx) => (
                    <button
                      key={`section-${idx}`}
                      className="editor-card"
                      onClick={() => onSelectSection(section.path)}
                    >
                      <strong>{section.label}</strong>
                      <span>Section background styles & elements</span>
                    </button>
                  ))}

                  {/* Footer Sections */}
                  {sections.filter(s => s.type === "footer").map((_, idx) => (
                    <button
                      key={`footer-${idx}`}
                      className="editor-card"
                    ...
                    >
                      ...
                      <span>Footer Layout, columns & bottom credits</span>
                    </button>
                  ))}
                </>
              ) : (
          ...
          {editorSubTab === "menus" && (
            <>
              ...
              <div style={{ fontSize: "11px", color: "var(--text-muted)", marginBottom: "16px", lineHeight: "1.4" }}>
                Click a menu below to edit its links. Drag items to reorder.
              </div>

              <div style={{ display: "flex", flexDirection: "column" }}>
                {navigationMenus.map((menu) => {
                  const items = typeof menu.items === 'string' ? JSON.parse(menu.items || '[]') : (menu.items || []);
                  const isExpanded = expandedMenuName === menu.name;

                  return (
                    <div className="menus-accordion-item" key={menu.name}>
                      <button 
                        className="menus-accordion-header"
                        onClick={() => {
                          setExpandedMenuName(isExpanded ? null : menu.name);
                          setEditingItemIndex(null);
                        }}
                        style={{ display: "flex", justifyContent: "space-between", width: "100%", padding: "12px 8px", background: "transparent", border: 0, borderBottom: "1px solid var(--border)", color: "white", cursor: "pointer", fontSize: "13px", fontWeight: "600" }}
                      >
                        <span>{menu.name}</span>
                      ...
                      {isExpanded && (
                        <div style={{ padding: "12px 8px", background: "rgba(0,0,0,0.15)", display: "flex", flexDirection: "column", gap: "10px" }}>
                          <div className="menu-buttons-list" style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {items.map((item: any, idx: number) => {
                              const isEditing = editingItemIndex === idx;
                              return (
                                <div key={idx} style={{ display: "flex", flexDirection: "column", width: "100%" }}>
                                  <div 
                                    className={`menu-item-btn ${isEditing ? 'active-edit' : ''}`}
                                    draggable={true}
                                    onDragStart={(e) => handleDragStartItem(e, idx)}
                                    onDragOver={(e) => handleDragOverItem(e, idx)}
                                    onDrop={(e) => handleDropItem(e, idx, menu.name)}
                                    onClick={(e) => {
                                      if ((e.target as HTMLElement).closest('.menu-item-close-icon')) return;
                                    ...
                                    style={{
                                      ...
                                      padding: "8px 12px",
                                      background: isEditing ? "rgba(249, 115, 22, 0.15)" : "rgba(255, 255, 255, 0.04)",
                                  ...
                                  >
                                    <div style={{ display: "flex", alignItems: "center" }}>
                                      <span className="menu-item-drag-icon" style={{ display: "inline-flex", marginRight: "6px", cursor: "grab" }}>
                                    ...
                                    <span 
                                      ...
                                      onClick={() => handleDeleteLink(menu.name, idx)}
                                      style={{ display: "inline-flex", cursor: "pointer", color: "#f87171" }}
                                    >
                                      <X size={12} />
                                    </span>
                                  </div>
                                  ...
                                  {isEditing && (
                                    <div className="menu-item-inline-edit" style={{ background: "rgba(255, 255, 255, 0.02)", border: "1px solid var(--border)", borderRadius: "6px", padding: "10px", display: "flex", flexDirection: "column", gap: "8px", marginTop: "4px", marginBottom: "8px" }}>
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        <label style={{ fontSize: '10px', color: 'var(--text-muted)' }}>Link Label</label>
                                        <input
                                          type="text"
                                          value={item.label}
                                          onChange={(e) => handleItemFieldChange(menu.name, idx, 'label', e.target.value)}
                                          style={{
                                      ...
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        <label style={{ fontSize: '10px', color: 'var(--text-muted)' }}>URL Path / Slug</label>
                                        <input
                                          type="text"
                                          value={item.url}
                                          onChange={(e) => handleItemFieldChange(menu.name, idx, 'url', e.target.value)}
                                          style={{
                                            background: 'rgba(0,0,0,0.3)',
                                            border: '1px solid var(--border)',
                                            borderRadius: '4px',
                                      ...
                                      <button 
                                        ...
                                        style={{ alignSelf: 'flex-end', marginTop: '4px', padding: '4px 10px', fontSize: '11px', borderRadius: '4px', cursor: "pointer" }}
                                      >
                                        Done
                                      </button>
                                    </div>
                          ...
                          <button
                            className="small-btn primary"
                            onClick={() => handleAddLink(menu.name)}
                            style={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: '6px',
  ...
  // 3.5. RAIL NAVIGATION EDITOR OUTLET
  if (subView === "rail") {
    return (
      <div className="sidebar-view">
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '24px' }}>
          <div className="editor-local-tabs" style={{ flex: 1, marginBottom: 0 }}>
            {["style", "layout", "effect"].map((tab) => (
              <button
                key={tab}
                className={`local-tab ${localTab === tab ? "active" : ""}`}
                onClick={() => setLocalTab(tab)}
        ...
        <div className="panel">
          ...
          {localTab === "layout" && (
            <>
              <div className="edit-box">
                <h4>Menu Position & Layout</h4>
                <p>Determine where the rail menu is placed and how it renders.</p>
                <div className="choice-grid">
                  {[
                    { key: "left", name: "Left Sidebar", desc: "Pinned vertical sidebar on the left side" },
                    { key: "right", name: "Right Sidebar", desc: "Pinned vertical sidebar on the right side" },
                    { key: "below-header", name: "Below Header", desc: "Horizontal dark menu strip below the header" }
                  ].map((item) => (
                    <button
                      key={item.key}
                      className={`visual-card ${state.railPosition === item.key || (!state.railPosition && item.key === 'left') ? "active" : ""}`}
                      onClick={() => {
              ...
              {state.railPosition === "below-header" && (
                <div className="edit-box" style={{ marginTop: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <h4 style={{ margin: 0 }}>Show Icons</h4>
                      <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--text-muted)' }}>Display icons next to navigation labels below the header.</p>
                    </div>
                    <button
          ...
          {localTab === "effect" && (
            <>
              <div className="edit-box">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <h4 style={{ margin: 0 }}>Enable Shadow</h4>
                    <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--text-muted)' }}>Add shadow effect to the rail menu.</p>
                  </div>
                  <button
              ...
              {state.railShadow && (
                <>
                  <div className="edit-box" style={{ marginTop: '20px' }}>
                    ...
                    <div className="choice-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                      {[
                        ...
                        <button
                          key={item.key}
                          className={`visual-card ${state.railShadowIntensity === item.key || (!state.railShadowIntensity && item.key === 'medium') ? "active" : ""}`}
                          onClick={() => onChange({ railShadowIntensity: item.key })}
                          style={{ padding: '8px', minHeight: 'auto', textAlign: 'center' }}
                        >
                          <strong>{item.name}</strong>
                        </button>
                  ...
                  <div className="edit-box" style={{ marginTop: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <h4 style={{ margin: 0 }}>Themed Shadow</h4>
                        <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--text-muted)' }}>Use accent color for the shadow color.</p>
                      </div>
                      <button
              ...
              <div className="edit-box" style={{ marginTop: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <h4 style={{ margin: 0 }}>Enable Border</h4>
                    <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--text-muted)' }}>Add outline border around the rail menu.</p>
                  </div>
                  <button
                  ...
                  <div className="edit-box" style={{ marginTop: '20px' }}>
                    ...
                    <div className="choice-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                      {[
                        ...
                        <button
                          key={item.key}
                          className={`visual-card ${state.railBorderSize === item.key || (!state.railBorderSize && item.key === 'small') ? "active" : ""}`}
                          onClick={() => onChange({ railBorderSize: item.key })}
                          style={{ padding: '8px', minHeight: 'auto', textAlign: 'center' }}
                        >
                          <strong>{item.name}</strong>
                        </button>
                  ...
                  <div className="edit-box" style={{ marginTop: '20px' }}>
                    ...
                    <div className="choice-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                      {[
                        ...
                        <button
                          key={item.key}
                          className={`visual-card ${state.railBorderColor === item.key || (!state.railBorderColor && item.key === 'standard') ? "active" : ""}`}
                          onClick={() => onChange({ railBorderColor: item.key })}
                          style={{ padding: '8px', minHeight: 'auto', textAlign: 'center' }}
                        >
                          <strong>{item.name}</strong>
                        </button>
          ...
          <button className="small-btn" style={{ width: "100%", marginTop: "24px" }} onClick={() => setSubView("sections")}>
            Back to Sections
          </button>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="edit-box">
                ...
                <div className="choice-grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
                  {[
                    { key: 4, name: "4 Columns", desc: "Explore, Connect, Service Times" },
                    { key: 5, name: "5 Columns", desc: "Explore, Connect, Footer 3, Service Times" },
                  ].map((item) => (
                    <button
                      key={item.key}
                      className={`visual-card ${(state.footerColumns || 4) === item.key ? "active" : ""}`}
                      onClick={() => onChange({ footerColumns: item.key })}
                      style={{ padding: "10px" }}
                    >
                      <strong>{item.name}</strong>
                      <span style={{ fontSize: "10px", marginTop: "2px" }}>{item.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
