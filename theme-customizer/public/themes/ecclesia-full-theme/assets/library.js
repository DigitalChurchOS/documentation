...
if(grid){
 ...
 function apply(){const q=(search.value||'').toLowerCase(),c=cat.value,t=type.value,a=access.value,m=metas();let visible=0;Array.from(grid.children).forEach(card=>{const ok=(!q||card.dataset.title.toLowerCase().includes(q)||card.dataset.tags.toLowerCase().includes(q))&&(!c||card.dataset.category===c)&&(!t||card.dataset.type===t)&&(!a||card.dataset.access===a)&&(!m.length||m.every(x=>card.dataset.tags.includes(x)));card.style.display=ok?'':'none';if(ok)visible++});Array.from(grid.children).sort((a,b)=>sort.value==='oldest'?Number(b.dataset.order)-Number(a.dataset.order):Number(a.dataset.order)-Number(b.dataset.order)).forEach(card=>grid.appendChild(card));count.textContent=visible+' resources';empty.classList.toggle('show',visible===0)}
