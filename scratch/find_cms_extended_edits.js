...
for (const line of lines) {
  ...
  try {
    ...
    for (const tc of toolCalls) {
      ...
      if (targetFile.includes('cms_extended.test.ts')) {
        console.log(`Step ${data.step_index} - Tool: ${tc.name}`);
        const outPath = `C:\\Users\\Administrator\\Documents\\ChurchOS\\scratch\\edit_cms_extended_step_${data.step_index}.json`;
