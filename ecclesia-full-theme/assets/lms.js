...
if(grid){
 ...
 function apply(){const q=(search.value||'').toLowerCase(),t=track.value,l=level.value,f=format.value,s=status.value,m=metas();let visible=0;Array.from(grid.children).forEach(card=>{const ok=(!q||card.dataset.title.toLowerCase().includes(q)||card.dataset.tags.toLowerCase().includes(q)||card.dataset.instructor.toLowerCase().includes(q))&&(!t||card.dataset.track===t)&&(!l||card.dataset.level===l)&&(!f||card.dataset.format===f)&&(!s||card.dataset.status===s)&&(!m.length||m.every(x=>card.dataset.tags.includes(x)));card.style.display=ok?'':'none';if(ok)visible++});Array.from(grid.children).sort((a,b)=>sort.value==='oldest'?Number(b.dataset.order)-Number(a.dataset.order):Number(a.dataset.order)-Number(b.dataset.order)).forEach(c=>grid.appendChild(c));count.textContent=visible+' courses';empty.classList.toggle('show',visible===0)}
