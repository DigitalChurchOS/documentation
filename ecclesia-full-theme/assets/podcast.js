...
if(grid){
 ...
 function apply(){const q=(search.value||'').toLowerCase(),c=channel.value,s=speaker.value,m=metas();let visible=0;Array.from(grid.children).forEach(card=>{const ok=(!q||card.dataset.title.toLowerCase().includes(q)||card.dataset.speaker.toLowerCase().includes(q)||card.dataset.tags.toLowerCase().includes(q))&&(!c||card.dataset.channel===c)&&(!s||card.dataset.speaker===s)&&(!m.length||m.every(x=>card.dataset.tags.includes(x)));card.style.display=ok?'':'none';if(ok)visible++});Array.from(grid.children).sort((a,b)=>sort.value==='oldest'?Number(b.dataset.order)-Number(a.dataset.order):Number(a.dataset.order)-Number(b.dataset.order)).forEach(card=>grid.appendChild(card));count.textContent=visible+' episodes';empty.classList.toggle('show',visible===0)}
